...
| Vendor SOC 2 attestation collection | Sub-processor list exists at `/legal/privacy`; signed DPA + SOC 2 reports are not yet stored in a versioned vault. | Operator + Engineering - establish the vendor evidence vault before the audit window. |
...
| Disaster-recovery runbook + tested restore | `docs/runbooks/` contains operational runbooks but no tested DR cycle is recorded. | Engineering - quarterly restore drill, documented in `docs/runbooks/`. |
...
5. `docs/adr/0016-pii-vault-envelope-encryption.md` - design rationale for envelope encryption.
6. `docs/runbooks/` - operational runbooks (incident response, restore, key rotation).
