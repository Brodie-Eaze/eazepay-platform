...
if (!res.ok) {
  ...
  setError(body.code === 'invalid_credentials'
...
4. **Adverse Action Notice retention.** Today retention is hardcoded to 25 months (`compliance-doc.service.ts:14`). This satisfies Reg B / FCRA decline retention but does not address the *loan agreement* retention (7+ years). Confirm the document-kind retention matrix and parameterise it.
