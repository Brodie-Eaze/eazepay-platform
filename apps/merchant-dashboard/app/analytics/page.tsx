    ...
    <>
      ...
      <PageBody>
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-4">
          <KpiCard label="Approval rate (30d)" value="64%" delta={{ value: '+5pp', direction: 'up', isGood: true }} hint="approved / submitted" />
          <KpiCard label="Attach rate (30d)" value="42%" delta={{ value: '+3pp', direction: 'up', isGood: true }} hint="financed / total sales" />
          <KpiCard label="Avg sale (financed)" value={<Money cents={2_240_000} compact />} delta={{ value: '+8%', direction: 'up', isGood: true }} />
          <KpiCard label="Median time-to-fund" value="2h 14m" delta={{ value: '-18m', direction: 'down', isGood: true }} />
        </div>
          ...
          <Card>
            ...
            <CardBody>
              <div className="text-accent">
                <BarChart data={trend.map((v, i) => ({ label: i % 5 === 0 ? `${i + 1}d` : '', value: v }))} height={160} />
              </div>
            </CardBody>
          </Card>
          <Card>
                  ...
                  return (
                    <div key={s.label}>
                      <div className="flex items-center justify-between text-[13px] mb-1">
                        <span>{s.label}</span>
                        <span className="tabular-nums">{s.value} · {pct.toFixed(0)}%</span>
                      </div>
