...
const columns: Column<ApplicationLink>[] = [
  ...
  },
  {
    ...
    cell: (l) =>
      l.saleAmountCents ? (
        <Money cents={l.saleAmountCents} noFractions />
      ) : (
  ...
  },
  {
    key: 'customer',
    header: 'Customer',
    cell: (l) => <span className="text-[12px]">{l.customerEmail ?? '—'}</span>,
  },
  {
    key: 'metrics',
    header: 'Funnel',
    ...
    cell: (l) => (
      <span className="text-[12px] tabular-nums text-fg-muted">
        {l.views} · {l.starts} · {l.submitted} · {l.funded}
      </span>
    ),
  },
  {
    ...
    cell: (l) =>
      l.status === 'active' ? (
        ...
        <StatusPill tone="info">Used</StatusPill>
      ) : (
  ...
  },
  {
    ...
    align: 'right',
    cell: (l) => (
      <span className="text-[12px] text-fg-muted tabular-nums">{fmtDate(l.expiresAt)}</span>
    ),
  },
  {
    key: 'actions',
    ...
    cell: () => (
      <div className="flex items-center gap-1 justify-end">
        <Button size="sm" variant="ghost" leadingIcon={<CopyIcon size={12} />}>
          Copy
        </Button>
      ...
      <PageBody>
        <Card className="mb-4">
          <CardHeader
            ...
            description="Pre-fill the sale to remove friction. The link is signed, expiring, and merchant-scoped."
          />
          <CardBody>
            ...
            <div className="mt-3">
              <Input
                label="Product / job note"
                placeholder="11.6 kW system · 13.5 kWh battery · permit included"
              />
            </div>
          </CardBody>
          <CardFooter>
        ...
        <Card className="mt-4">
          <CardHeader
            title="Embed at checkout"
            description="Drop the EazePay widget into your checkout to offer financing without redirecting customers."
          />
          <CardBody>
