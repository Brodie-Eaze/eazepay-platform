    ...
    cell: (s) =>
      s.paidAt ? (
        <span className="text-[12px] text-fg-muted tabular-nums">
          {new Date(s.paidAt).toLocaleString('en-US')}
        </span>
      ) : (
        '—'
      ),
      ...
      <PageBody>
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-4">
          ...
          <KpiCard
            ...
            value={
              <Money
                cents={settlements.find((s) => s.status === 'in_flight')?.netCents ?? 0}
                compact
              />
            }
            hint="settled to your bank today"
          />
          <KpiCard
            label="MDR paid (MTD)"
            value={
              <Money
                cents={settlements
                  .filter((s) => s.status === 'paid')
                  .reduce((a, s) => a + s.mdrCents, 0)}
              />
            }
            hint="285 bps · effective"
          />
          <KpiCard label="Reconciliation health" value="100%" hint="0 breaks" />
        </div>

        <Banner intent="info" className="mb-4">
          Payouts use RTP same-day when available, with same-day ACH fallback. A signed receipt for
          each transfer is delivered via webhook and stored in your settlements log for 7 years.
        </Banner>
