...
const STEPS: StepView[] = EZ_CHECK_MODULES.map((m) => ({
  ...
  // CORE points at the wizard sign-up; HELIX + ORACLE point at the
  // admin panels (operators still configure those server-side today).
  configHref:
    m.id === 'core'
      ? '/welcome'
      : `/admin?surface=ez-check&panel=${m.id === 'helix' ? 'SmartForm' : 'PreQualAgents'}`,
}));
    ...
    <div className="ezk-onb-root">
      ...
      <header className="ezk-onb-nav">
        <div className="ezk-onb-nav-inner">
          <Link href="/landing/ez-check" className="ezk-onb-brand" aria-label="EZ Check home">
