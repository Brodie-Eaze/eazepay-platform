...
describe('POST /api/integrations/highsale/prequal — FCRA consent gate', () => {
  ...
  it('412 expired — consent older than the 30-day freshness window', async () => {
    // Capture a receipt, then back-date its timestamp past the window by
    ...
    // production the timestamp comes from Postgres and is immutable.
    receipt.timestamp = new Date(staleMs).toISOString();
