/**
 * MedPay · Checkout (/medpay/checkout)
 *
 * Aurean-style conversion page. The prospect lands here from
 * /medpay/start, the sales deck final CTA, or the /landing/medpay
  ...
  async function payWithStripe() {
    ...
    try {
      const res = await fetch('/api/billing/stripe/create-setup-session', {
        ...
        body: JSON.stringify({ brand: 'medpay', businessName, billingEmail }),
      });
      const data = await res.json();
      if (data?.url) {
        // Live Stripe Checkout — redirect to the hosted session.
        window.location.href = data.url as string;
        return;
      }
      if (data?.stub && data?.redirect) {
        // Placeholder mode — Stripe keys not yet wired. Continue to onboarding.
        setStripeNote(
        ...
        <section className="mpf-hero">
          <div className="mpf-container">
            <div className="mpf-eyebrow-pill">
              ...
              Activate MedPay · Step 1 of 2
            </div>
            <h1 className="mpf-h1">
              <span className="mpf-grad-teal">Activate MedPay</span>
              <br />
              <span className="mpf-grad-teal-deep">for your practice.</span>
            </h1>
          ...
          <div className="mpf-container">
            <div className="mpf-pricing-grid">
              ...
              <PricingTier
                ...
                body="At the end of each month we invoice 4% of loans that actually settled. No funded loan → no origination fee. Tracked per-lender, per-practice."
                v="4%"
            ...
            <div className="mpf-co-wrap">
              <div className="mpf-co-l">
                ...
                <ul className="mpf-co-points">
                  <li>
                    ...
                    originator. Lenders carry the credit risk; the practice does not.
                  </li>
                  <li>
                    <span className="mpf-co-check" aria-hidden>
                      ✓
                  ...
                  <li>
                    ...
                    any time. Existing loans funded under MedPay continue under their lender terms.
                  </li>
                  <li>
                    <span className="mpf-co-check" aria-hidden>
                      ✓
              ...
              <form className="mpf-co-form" onSubmit={(e) => e.preventDefault()}>
                ...
                <label className="mpf-co-field">
                  ...
                  <input
                    type="email"
                    placeholder="billing@helio.dental"
                    value={billingEmail}
...
.mpf-co-form {
  padding: 24px;
  background:
    radial-gradient(ellipse 80% 100% at 100% 0%, rgba(34, 184, 160, 0.12), transparent 70%),
    rgba(255, 255, 255, 0.98);
  border: 1px solid var(--mp-line-strong);
  border-radius: 18px;
  display: flex; flex-direction: column; gap: 14px;
  box-shadow: 0 28px 60px -28px rgba(14, 124, 102, 0.32);
}
...
.mpf-stripe-block {
  ...
  padding: 14px;
  background:
    radial-gradient(ellipse 100% 100% at 0% 0%, rgba(14, 124, 102, 0.10), transparent 70%),
