/**
 * CoachPay · Onboarding wizard (/coachpay/onboarding)
 *
 * 5-step KYB form. Aurean-style chrome (.mpf-* namespace) + CoachPay
 * dark purple palette. Form state is local (useState); on submit the
 * user lands at /coachpay/success.
 *
 * Step 1 · Business info     · legal name, EIN, coaching type, address
 * Step 2 · Primary contact   · owner / operator name, email, phone
 * Step 3 · Banking + payout  · routing + account, payout cadence
 * Step 4 · Documents         · business reg, E&O insurance, W-9, voided check
 * Step 5 · Review + e-sign   · typed signature, MSA acceptance
 *
 * On submit we don't actually POST anywhere yet — this is a UI
 * scaffold that wires to /api/onboarding/coachpay later. The flow
...
export default function CoachPayOnboarding(): JSX.Element {
  ...
  const v4 = s4.providerLicense && s4.eoInsurance && s4.w9 && s4.voidedCheck;
  ...
  function next() {
    ...
    else router.push('/coachpay/success');
  }
  function back() {
    ...
    <div className="mpf-root">
      ...
      <header className="mpf-nav">
        <div className="mpf-container mpf-nav-inner">
          <Link href="/coachpay/start" className="mpf-brand" aria-label="CoachPay home">
            <span className="mpf-brand-mark">
              <LogoIcon />
            </span>
            <span className="mpf-brand-word">
              CoachPay<span className="mpf-brand-sub">/Client Financing</span>
            </span>
          </Link>
        ...
        <section className="mpf-hero mpf-hero-tight">
          <div className="mpf-container">
            <div className="mpf-eyebrow-pill">
              ...
              Activate CoachPay · Step 2 of 2
            </div>
            <h1 className="mpf-h1">
              <span className="mpf-grad-teal">Get your coaching business live</span>
              <br />
      ...
      <footer className="mpf-footer">
        <div className="mpf-container mpf-footer-inner">
          <div className="mpf-footer-brand">
            <LogoIcon />
            <span>CoachPay · A vertical of EazePay</span>
          </div>
...
function Step1Form({ s, set }: { s: Step1; set: (x: Step1) => void }) {
  return (
    <div className="mpf-wiz-body-inner">
      <h2 className="mpf-h2">Tell us about your coaching business</h2>
      <p className="mpf-section-sub">
        We pull the EIN against IRS records (60 seconds). For high-ticket coaching there is no
        state-board license, so we instead verify business registration + W-9 + E&O insurance.
      </p>
      <div className="mpf-fld-grid">
        <Field label="Legal business name">
          <input
            type="text"
            placeholder="Beck Executive Studio, LLC"
            value={s.legalName}
        ...
        <Field label="Coaching type">
          ...
          >
            <option value="executive">Executive coaching · 1:1</option>
            <option value="mastermind">Masterminds + cohorts</option>
            <option value="course">Cohort courses · certification</option>
            <option value="consulting">Consulting retainers · fractional CXO</option>
            <option value="health">Health + performance + longevity</option>
          </select>
        </Field>
...
function Step2Form({ s, set }: { s: Step2; set: (x: Step2) => void }) {
  return (
    <div className="mpf-wiz-body-inner">
      ...
      <p className="mpf-section-sub">
        Who signs the lender agreement and receives the funded-loan reports? Usually the coach owner
        or a senior operator. They&apos;ll get the partner-portal admin seat.
      </p>
      <div className="mpf-fld-grid">
        <Field label="Full name">
          <input
            type="text"
            placeholder="Lena Beck"
            value={s.ownerName}
        ...
        <Field label="Role">
          <select value={s.ownerRole} onChange={(e) => set({ ...s, ownerRole: e.target.value })}>
            ...
            <option>Head closer / Sales director</option>
        ...
        <Field label="Email">
          <input
            type="email"
            placeholder="lena@beckexecutive.com"
            value={s.ownerEmail}
...
function Step4Form({ s, set }: { s: Step4; set: (x: Step4) => void }) {
  const docs: Array<{ k: keyof Step4; label: string; body: string }> = [
    {
      k: 'providerLicense',
      label: 'Business registration / articles',
      body: 'Articles of incorporation, LLC formation, or DBA registration for the coaching entity.',
    },
    {
      k: 'eoInsurance',
      label: 'Errors & omissions (E&O) insurance',
      body: 'Active COI showing the coaching business carries professional E&O coverage. Recommended at $1M+ for high-ticket masterminds.',
    },
    {
      k: 'w9',
      label: 'W-9 (signed)',
      body: 'Current W-9 for the coaching entity, matching the EIN entered on step 1.',
    },
    {
      k: 'voidedCheck',
      label: 'Voided check or bank letter',
      body: 'Confirms the routing + account number entered on step 3 belong to the coaching business.',
    },
  ];
  return (
    <div className="mpf-wiz-body-inner">
    ...
    <div className="mpf-wiz-body-inner">
      ...
      <div className="mpf-review">
        <ReviewBlock title="Business">
          ...
          <ReviewRow k="Coach type" v={all.s1.practiceType} />
          <ReviewRow
            k="Address"
...
const CSS = `
.mpf-root {
  /* CoachPay palette — dark theme, purple primary */
  --mp-teal: #8B5CF6;
  --mp-teal-2: #A78BFA;
...
@keyframes mpfPulse { 0%,100% { box-shadow: 0 0 0 0 rgba(167, 139, 250, 0.55); } 50% { box-shadow: 0 0 0 6px rgba(167, 139, 250, 0); } }
.mpf-h1 {
  margin: 18px 0 14px;
...
.mpf-btn-primary {
  ...
  box-shadow: 0 12px 24px -8px rgba(139, 92, 246, 0.45);
  transition: transform .15s ease, box-shadow .15s ease;
}
.mpf-btn-primary:hover:not(:disabled) {
  transform: translateY(-2px);
  box-shadow: 0 16px 32px -8px rgba(139, 92, 246, 0.55);
}
