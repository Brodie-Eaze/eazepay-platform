...
export default function MedPayApplyPage() {
  ...
  // Preview-mode short-circuit so you can share a direct landing link
  // for the offers card (e.g. lender-logo screenshot, sales demo). The
  // intake form still drives the real flow; this only seeds defaults
  // when ?preview=offers is set.
  //   /apply/medpay?preview=offers&amount=20000&tier=sub_prime
  ...
  useEffect(() => {
    ...
    // Dual-write: also POST to the server-side database. The fetch is
    // fire-and-forget and degrades cleanly to localStorage-only when
    // DATABASE_URL is not yet provisioned (503 db_unavailable).
    submitApplicationToApi({
      brand: 'medpay',
      partnerId,
