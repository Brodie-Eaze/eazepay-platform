...
const reviews = [
  ...
  },
  {
    ...
    status: 'open',
    assignee: 'Devon L.',
    opened: '2026-05-04T13:48Z',
  },
  {
    id: 'cr_R8Kv',
    subject: 'app_9xQpL4mNkjT',
    kind: 'fair_lending_audit',
    status: 'closed',
    assignee: 'Priya V.',
    opened: '2026-05-03T11:30Z',
  },
  {
    id: 'cr_8RKv',
    subject: 'cust_8mRT2WQpKvN',
    kind: 'cfpb_complaint',
    status: 'open',
    assignee: 'Devon L.',
    opened: '2026-05-02T16:21Z',
  },
];
    ...
    <>
      ...
      <PageBody>
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-4">
          <KpiCard
            label="Open reviews"
            value={reviews.filter((r) => r.status === 'open').length}
            hint="all severities"
          />
          <KpiCard label="SAR queue" value="1" hint="30-day filing window" />
          <KpiCard
            label="CFPB complaints (60d)"
            value="3"
          ...
          <KpiCard
            ...
            value="2.1d"
            delta={{ value: '-0.6d', direction: 'down', isGood: true }}
          />
        </div>

        <Banner intent="info" className="mb-4">
          Closing a review requires (a) reviewer rationale, (b) second-reviewer signoff, and (c)
          attachment of any artifact filed externally (FinCEN BSA E-File ack, CFPB response). All
          hashed to the audit chain.
        </Banner>
          ...
          {reviews.map((r) => (
            <Card key={r.id}>
              <CardHeader
                ...
                description={
                  <>
                    Review <span className="font-mono">{r.id}</span> · subject {r.subject}
                  </>
                }
                action={
                  r.status === 'open' ? (
              ...
              <CardBody>
                ...
                <DataRow
                  ...
                  value={
                    r.kind === 'sar_prefile'
                      ? '2026-05-29 (FinCEN 30d)'
                      : r.kind === 'cfpb_complaint'
                        ? '2026-05-07 (CFPB 15d initial)'
                        : '—'
                  }
                />
                {r.status === 'open' && (
