...
export default function EzCheckCheckout(): JSX.Element {
  ...
  /**
   * Standalone-app checkout: no Stripe yet. We post the lead to the
   * launch team via mailto + forward the operator straight to the
   * onboarding screen. The $5,000 setup fee is invoiced once the
   * launch engineer has the business details. Wire Stripe later via
   * a dedicated /api/checkout route on this same service.
   */
  async function payWithStripe() {
    if (!canProceed || paying) return;
    setPaying(true);
    setStripeNote('Setup fee will be invoiced by the launch team. Continuing to onboarding…');
    setTimeout(() => {
    ...
    <div className="ezk-root">
      ...
      <header className="ezk-nav">
        <div className="ezk-container ezk-nav-inner">
          <Link href="/" className="ezk-brand" aria-label="EZ Check home">
          ...
          <div className="ezk-nav-cta-group">
            <a
              href="mailto:launch@eazepay.com?subject=EZ%20Check%20—%20enquiry"
              className="ezk-nav-link"
            >
              Contact sales
            </a>
          </div>
          ...
          <div className="ezk-container">
            <div className="ezk-co-wrap">
              <div className="ezk-co-l">
                ...
                <a
                  href="mailto:launch@eazepay.com?subject=EZ%20Check%20—%20MSA%20request"
                  className="ezk-co-fine"
                >
              ...
              <form className="ezk-co-form" onSubmit={(e) => e.preventDefault()}>
                ...
                <div className="ezk-stripe-block">
                  ...
                  >
                    {paying ? 'Forwarding to onboarding…' : 'Start onboarding · invoice me $5,000'}
                    <ArrowIcon />
                  </button>
                  {stripeNote ? <div className="ezk-stripe-note">{stripeNote}</div> : null}
                </div>
                <div className="ezk-co-foot">
                  Setup fee invoiced by the launch team · Live in up to 5 business days
                </div>
              </form>
            </div>
          </div>
